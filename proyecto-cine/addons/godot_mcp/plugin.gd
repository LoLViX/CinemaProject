...
func _enter_tree():
	...
	# Update panel to reflect that server is already running
	await get_tree().process_frame
	mcp_panel.update_status("Running on port %d" % SERVER_PORT)
	mcp_panel.start_button.disabled = true
	mcp_panel.stop_button.disabled = false
	mcp_panel.add_log_message("MCP Server started on port %d" % SERVER_PORT)
